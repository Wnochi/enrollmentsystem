...
1. Audit existing tables and policies. The included migration is additive and uses the `es_` prefix to avoid replacing existing data.
2. Review and run `supabase/migrations/202609110001_enrollment_system.sql` in the Supabase SQL editor or with the Supabase CLI.
3. Copy `.env.example` to `.env.local` and fill in the project URL and public anon key. Never put the service-role key in the browser app.
